...
def test_empty_hours_returns_none():
    assert analyze_clothing([], "off") is None


def test_hot_summer_day_off_work():
    hours = [
    ...
    result = analyze_clothing(hours, work_mode="off")
    ...
    assert result["commute_advice"] == []


def test_cold_rainy_windy_office_day_has_commute_advice():
    hours = [
    ...
    result = analyze_clothing(hours, work_mode="office")
    ...
    assert result["commute_advice"] == [
        i18n.t("commute_rain_morning", hour=8),
        i18n.t("commute_cold_morning", feels=2),
        i18n.t("commute_wind_morning"),
        i18n.t("commute_rain_evening", hour=18),
    ]


def test_remote_day_has_no_commute_advice_even_if_conditions_match():
    hours = [
        {"hour": 8, "temp": 3, "feels_like": 2, "rain_prob": 90, "wind": 40, "uv": 0, "code": 61},
    ]

    result = analyze_clothing(hours, work_mode="remote")
