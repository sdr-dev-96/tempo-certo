...
2. Set that value as `TELEGRAM_BOT_TOKEN` in `config.py`.
...
Fill in the `SMTP_*` fields in `config.py` and set `NOTIFY_METHOD = "email"`. For
Gmail, you'll need to generate an "app password" (not your regular password):
...
- `config.py` — all configuration (location, thresholds, work schedule, notification)
